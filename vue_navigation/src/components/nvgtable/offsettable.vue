<template>
  <el-table
    ...
    <el-table-column
      ...
      <template slot-scope="scope">
        <!--<i class="el-icon-time"></i>-->
        <span style="margin-left: 10px">{{ scope.row.offset }}</span>
      </template>
    </el-table-column>
    ...
    <el-table-column
      ...
      <template slot-scope="scope">
        <!--<i class="el-icon-time"></i>-->
        <span style="margin-left: 10px">{{ scope.row.coordinate }}</span>
      </template>
    </el-table-column>
    <el-table-column
      label="方向"
      ...
      <template slot-scope="scope">
        <!--<i class="el-icon-time"></i>-->
        <span style="margin-left: 10px">{{ scope.row.direction }}</span>
      </template>
    </el-table-column>
    <el-table-column
      label="长度"
        ...
        <el-dialog title="路名增加" :visible.sync="dialogFormVisible">
          <el-form :model="Form">
            ...
            <el-form-item label="坐标" :label-width="formLabelWidth">
              <el-input v-model="Form.coordinate" autocomplete="off">
              </el-input>
            </el-form-item>
...
<script>
  export default {
    ...
    methods: {
      ...
      postForm() {
        const url = this.HOST + '/navigation-web/offset/save';
        this.dialogFormVisible = false;
        var params = new URLSearchParams();
        params.append('name', this.Form.offset);
        params.append('start', this.Form.coordinate);
        params.append('end', this.Form.direction);
        params.append('length', this.Form.size);
        console.log(params);
        this.$axios({
          method: 'post',
          url: url,
      ...
      handleDelete(index, row) {
        ...
        console.log(placeId);
        this.$axios
          .delete(this.HOST + '/navigation-web/offset/delete/' + placeId)
          .then(res => {
            console.log(res);
    ...
    created() {
      this.$axios.get(this.HOST+'/navigation-web/offset/all')
        .then(response=>{
          console.log(response);
